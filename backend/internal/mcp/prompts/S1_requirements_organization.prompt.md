...
按照原文档的逻辑结构，分类整理需求。
**注意：** 每个一级需求 (REQ-XXX) 将作为一个独立的切分单元。请确保属于该需求的所有描述、子需求、表格或图片描述都完全包含在该标题层级之下。
...
- **代码友好性 (Parser Friendly):** 输出必须严格遵守结构，以便后续脚本自动拆分
    - 所有的需求内容必须包含在 `#### REQ-xxx` 的标题块下
    - **严禁**在两个需求标题之间插入不带编号的“过渡段落”或“解释文本”
    - 每个一级需求应当是一个独立的、完整的逻辑单元
- 保持原文档的逻辑顺序和层次结构
- 翻译必须准确流畅，符合中文表达习惯
