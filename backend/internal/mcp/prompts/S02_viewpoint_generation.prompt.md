...
* **强追溯性 (Strong Traceability - CRITICAL):** 
  - 每个测试观点必须关联到具体的需求编号
  - 需求编号格式：R{转换文档ID}-{序号}（如R28-001）
  - 观点编号格式：V{转换文档ID}-{序号}（如V28-001）
  - 确保追溯链完整：观点 → 需求 → 转换文档 → 原始文档
  - 如果观点无直接关联需求（如全系统联动功能），使用V{转换文档ID}-SYS-XXX格式，关联需求字段可留空

* **观点抽象性 (Abstract Viewpoints - CRITICAL):**
  - 每个观点应该是**抽象的测试观点**，而非详细的测试用例
  - 观点描述应简洁明确，指出需要验证的测试点
  - 不需要包含详细的前置条件、操作步骤、期望结果
  - 观点是测试设计的指导，后续可扩展为具体测试用例
  - 保持观点的高层次抽象，便于快速理解测试覆盖范围
  
  **正确示例（抽象测试观点）：**
  - ✅ "验证[VCC3]界面中"系统-系统详细"下方正确显示"日志收集"项"
  - ✅ "验证日志收集开始按钮按下时正确显示确认对话框"
  - ✅ "验证打开已存在的[zip]输出目录时显示正确的确认提示"
  
  **错误示例（过于详细的用例）：**
  - ❌ "验证在[ユーザー名]输入框输入有效用户名，在[パスワード]输入框输入正确密码，点击[ログイン]按钮后成功跳转到[首页]"（太具体，包含了操作步骤）
  - ❌ "前置条件：xxx，验证动作：1.xxx 2.xxx，期望结果：xxx"（这是测试用例格式，不是观点）
...
* **专有名词标识 (Term Identification):** 所有专有名词（尤其是UI元素）必须使用 `[术语]` 格式标识，保持术语一致性。

* **逐条填充Chunk原则 (Complete Output - CRITICAL):** 
  - 必须使用 `update_viewpoint_item` 逐个完整写入Chunk
  - 每个Chunk的内容必须完整，不得简化或摘要
  - 如果Token不足，**禁止为了节省Token而进行批量写入、摘要或简化内容**
  - 必须暂停并提示用户输入"继续"，然后继续处理下一个Chunk
  - Token使用率超过85%时立即暂停，输出进度报告
...
#### 8.3 Token限制处理 (CRITICAL - 最高优先级规则)

**核心原则（不可违背）：**

1. **逐条填充Chunk原则**：
   - ✅ 必须逐个Chunk完整生成内容并调用 `update_viewpoint_item` 写入
   - ✅ 每个Chunk的内容必须完整、不得简化或摘要
   - ❌ 严禁批量生成多个Chunk后一次性输出
   - ❌ 严禁跳过某些Chunk
   - ❌ 严禁使用"内容见附件"、"详见XX"等占位符

2. **Token即将耗尽时的处理流程**：
   ```
   步骤1: 检测到Token使用率超过85%时立即暂停
   步骤2: 完成当前Chunk的写入（不允许半途而废）
   步骤3: 按照"阶段3：中断阶段"格式输出详细进度报告
   步骤4: 明确提示："⏯️ 请输入'继续'以继续生成剩余{N}个Chunk"
   步骤5: 等待用户输入
   步骤6: 用户输入"继续"后，按照"阶段4：继续阶段"格式恢复
   步骤7: 从断点处精确恢复，继续逐个Chunk处理
   ```

3. **严格禁止的行为**：
   - ❌ 为节省Token而批量输出多个Chunk
   - ❌ 为节省Token而简化观点描述
   - ❌ 为节省Token而省略操作步骤或预期结果
   - ❌ 为节省Token而使用摘要代替完整内容
   - ❌ 未经用户确认就自动继续处理
   - ❌ 在Token耗尽前不暂停，导致内容截断

4. **断点续传机制**：
   - 保存状态：`current_chunk_index`, `generated_viewpoints`, `used_viewpoint_ids`
   - 恢复时从 `chunks[current_chunk_index + 1]` 开始
   - 继承已生成的观点编号集合，避免重复
   - 显示明确的进度：已完成X/Y个Chunk，还剩Z个

5. **质量优先于速度**：
   - 宁可多次"继续"，也不能牺牲内容完整性
   - 宁可用户多输入几次"继续"，也不能简化观点描述
   - 宁可生成时间长，也不能省略重要的验证步骤

**监控机制**：
- 每完成1个Chunk后检查Token使用情况
- Token使用率 > 85% → 立即暂停
- Token使用率 > 90% → 紧急暂停（即使Chunk未完成也要暂停）
...
* **1.1. 关联需求文档 (Related Requirement):** 注明本观点文档对应的需求文档名称和转换文档ID。
...
- **需求编号格式**：R{转换文档ID}-{序号}（如R28-001）
- **观点编号格式**：V{转换文档ID}-{序号}（如V28-001）
- 需求编号来自原始转换文档，观点编号关联到对应的需求编号
- 这样确保从观点→需求→原始文档的完整追溯链
...
**观点描述标准（CRITICAL）：**
- ✅ **保持抽象性**：观点应是简洁的测试点描述，不是详细的测试用例
- ✅ **明确验证目标**：清晰指出需要验证什么功能或行为
- ✅ **使用术语标识**：所有专有名词使用[]标识
- ❌ **避免详细步骤**：不要写"第1步xxx，第2步xxx"这样的操作流程
- ❌ **避免用例格式**：不要包含"前置条件"、"验证动作"、"期望结果"等用例结构

**正确示例（抽象测试观点）：**
- ✅ "验证[VCC3]界面中正确显示[日志收集]项"
- ✅ "验证点击[开始按钮]后显示正确的确认对话框"
- ✅ "验证在[注意事项]对话框点击[OK]后开始日志收集"
- ✅ "验证日志收集正确显示收集进度"

**错误示例（过于详细）：**
- ❌ "验证在[ユーザー名]输入框输入有效用户名，在[パスワード]输入框输入正确密码，点击[ログイン]按钮后成功跳转到[首页]"
- ❌ "前置条件：用户已登录，验证动作：1.打开设置页面 2.点击按钮，期望结果：显示成功提示"
...
| ------------- | ------------- | ----------- | ---- | ----- |
| V{doc_id}-001 | R{doc_id}-001 | [简洁的测试观点描述，指出验证的测试点] | 功能   | 高/中/低 |
| V{doc_id}-002 | R{doc_id}-001 | [简洁的测试观点描述，指出验证的测试点] | 功能   | 高/中/低 |

##### **3.1.2. [中功能名称 - 异常系需求] (二级功能)**

| 观点编号          | 关联需求          | 测试观点描述             | 观点类型 | 优先级   |
| ------------- | ------------- | ------------------ | ---- | ----- |
| V{doc_id}-101 | R{doc_id}-001 | [简洁的异常场景测试观点描述] | 异常处理 | 高/中/低 |
| V{doc_id}-102 | R{doc_id}-001 | [简洁的边界值测试观点描述] | 异常处理 | 高/中/低 |
...
**CRITICAL - 编号追溯体系：**
- **转换文档ID**：每个上传的原始需求文档（如Excel、Word等）在系统中有唯一的转换文档ID
- **需求编号**：R{转换文档ID}-{序号} （如R28-001表示转换文档28的第1条需求）
- **观点编号**：V{转换文档ID}-{序号} （如V28-001表示关联转换文档28的第1个观点）
- **追溯链**：观点V28-001 → 需求R28-001 → 转换文档ID=28 → 原始Excel/Word文档

**编号格式规范：**

| 观点类型       | 编号格式                         | 示例            | 关联需求示例 |
| ---------- | ---------------------------- | ------------- | -------- |
| 功能性观点（功能）  | V{转换文档ID}-{3位序号}             | V28-001       | R28-001  |
| 功能性观点（异常系） | V{转换文档ID}-{1XX序号}            | V28-101       | R28-001  |
| 非功能性观点     | V{转换文档ID}-NF-{质量特性缩写}-{3位序号} | V28-NF-PE-001 | R28-005  |
| 合规性观点      | V{转换文档ID}-STD-{3位序号}         | V28-STD-001   | R28-010  |
| 无关联需求观点    | V{转换文档ID}-SYS-{3位序号}         | V28-SYS-001   | -        |

* 对于无明确关联需求的观点（如全系统联动），使用 `V{转换文档ID}-SYS-xxx` 格式，关联需求列留空或填 `-`。
* **重要**：转换文档ID必须与原始需求文档对应，确保从观点能追溯到原始上传的需求文件。
...
- **需求文档名称**: 用户管理系统 v2.0
- **转换文档ID**: 28
- **需求编号格式**: R28-XXX（如R28-001表示转换文档28的第1条需求）
- **观点编号格式**: V28-XXX（如V28-001表示关联转换文档28的第1个观点）
- **追溯链**: V28-001 → R28-001 → 转换文档ID=28 → 原始需求Excel文档
- **关联时间**: 2026-02-11
...
| 观点编号 | 关联需求 | 测试观点描述 | 观点类型 | 优先级 |
|---------|---------|-------------|---------|-------|
| V28-001 | R28-001 | 验证使用正确的[用户名]和[密码]可以成功登录[系统] | 功能 | 高 |
| V28-002 | R28-001 | 验证登录成功后正确跳转到[首页] | 功能 | 高 |
| V28-003 | R28-001 | 验证登录失败时显示明确的错误提示 | 功能 | 高 |
| V28-004 | R28-001 | 验证连续3次登录失败后账户被锁定 | 功能 | 高 |
| V28-005 | R28-001 | 验证账户锁定15分钟后自动解锁 | 功能 | 高 |
| V28-006 | R28-001 | 验证[密码]输入时正确显示遮罩符号 | 功能 | 中 |
| V28-007 | R28-001 | 验证"记住我"功能正确保持登录状态7天 | 功能 | 中 |
| V28-008 | R28-001 | 验证点击"忘记密码"链接可以进入密码重置流程 | 功能 | 中 |
```
...
| 观点编号 | 关联需求 | 测试观点描述 | 观点类型 | 优先级 |
|---------|---------|-------------|---------|-------|
| V28-101 | R28-001 | [边界值]验证[用户名]长度为3字符时的登录处理 | 异常处理 | 中 |
| V28-102 | R28-001 | [边界值]验证[用户名]长度为20字符时的登录处理 | 异常处理 | 中 |
| V28-103 | R28-001 | [越界]验证[用户名]长度小于3字符时拒绝登录 | 异常处理 | 中 |
| V28-104 | R28-001 | [越界]验证[用户名]长度超过20字符时拒绝登录 | 异常处理 | 中 |
| V28-105 | R28-001 | [等价类]验证[用户名]包含特殊字符时的处理 | 异常处理 | 中 |
| V28-106 | R28-001 | [等价类]验证[用户名]为空时显示必填提示 | 异常处理 | 高 |
| V28-107 | R28-001 | [等价类]验证[密码]为空时显示必填提示 | 异常处理 | 高 |
| V28-108 | R28-001 | [异常场景]验证第3次登录失败时显示锁定提示 | 异常处理 | 高 |
| V28-109 | R28-001 | [异常场景]验证锁定期间尝试登录时提示剩余锁定时间 | 异常处理 | 中 |
| V28-110 | R28-001 | [异常场景]验证网络中断时显示网络错误提示 | 异常处理 | 中 |
| V28-111 | R28-001 | [状态转换]验证从未登录状态到已登录状态的正确转换 | 状态转换 | 高 |
| V28-112 | R28-001 | [UI交互]验证点击取消按钮可以清空输入框 | 功能 | 低 |
```
