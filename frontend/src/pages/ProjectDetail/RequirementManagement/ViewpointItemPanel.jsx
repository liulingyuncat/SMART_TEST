...
import {
  ...
  updateViewpointItem,
  deleteViewpointItem,
  exportViewpointItemsToZip,
} from '../../../api/viewpointItem';
import {
  fetchChunks as fetchViewpointChunks,
  createChunk as createViewpointChunk,
  updateChunk as updateViewpointChunk,
  deleteChunk as deleteViewpointChunk,
} from '../../../api/viewpointChunk';
...
import './RequirementItemPanel.css'; // 复用相同样式

/**
 * AI观点条目管理面板 (T42)
 * 左侧列表 + 右侧Markdown编辑器
 */
const ViewpointItemPanel = ({ projectId, projectName }) => {
  ...
  const loadChunks = useCallback(async (itemId) => {
    ...
    try {
      const data = await fetchViewpointChunks(projectId, itemId);
  ...
  const loadItems = useCallback(async () => {
    setLoading(true);
    try {
      const data = await fetchViewpointItems(projectId);
      ...
      message.error(t('viewpoint.loadFailed'));
    } finally {
      setLoading(false);
    }
  }, [projectId, selectedItem, t, loadChunks]);
  ...
  const handleAddChunk = useCallback(async () => {
    ...
    try {
      const newChunk = await createViewpointChunk(projectId, selectedItem.id, '', '');
  ...
  const handleDeleteChunk = useCallback(async (chunkId) => {
    try {
      await deleteViewpointChunk(chunkId);
  ...
  const handleSaveChunk = useCallback(async (chunkId, title, content) => {
    try {
      const updated = await updateViewpointChunk(chunkId, title, content);
  ...
  const handleDownloadChunks = useCallback((item) => {
    ...
    a.href = url;
    a.download = `${item?.name || 'viewpoint'}.md`;
    a.click();
  ...
  const handleSaveItemName = async () => {
    ...
    try {
      await updateViewpointItem(selectedItem.id, editingItemName, selectedItem.content);
  ...
  const handleCreateItem = async () => {
    if (!newItemName.trim()) {
      message.warning(t('viewpoint.nameRequired'));
      return;
    }

    try {
      await createViewpointItem(projectId, newItemName, '');
      message.success(t('viewpoint.createSuccess'));
      setIsModalVisible(false);
      setNewItemName('');
      await loadItems();
    } catch (error) {
      message.error(t('viewpoint.createFailed'));
    }
  };
  ...
  const handleSave = async () => {
    if (!selectedItem) {
      message.warning(t('viewpoint.selectItemFirst'));
      return false;
    }

    if (!selectedItem.name.trim()) {
      message.warning('观点名称不能为空');
      return false;
    }

    try {
      await updateViewpointItem(selectedItem.id, selectedItem.name, editingContent);
      message.success(t('viewpoint.saveSuccess'));
      setIsEditMode(false);
      ...
      message.error(t('viewpoint.saveFailed'));
      return false;
    }
  };
  ...
  const handleDelete = async (itemId) => {
    try {
      await deleteViewpointItem(itemId);
      message.success(t('viewpoint.deleteSuccess'));
      ...
      message.error(t('viewpoint.deleteFailed'));
    }
  };

  // 版本保存（ZIP打包）
  const handleSaveVersion = async () => {
    console.log('=== [Viewpoint handleSaveVersion] 开始保存版本 ===');
    console.log('[Viewpoint] projectId:', projectId);
    console.log('[Viewpoint] items.length:', items.length);
    
    // 确保items是真正的数组
    const itemsArray = Array.isArray(items) ? items : Array.from(items || []);
    
    if (!itemsArray || itemsArray.length === 0) {
      console.warn('[Viewpoint] items为空，终止执行');
      message.warning('没有观点条目，无法保存版本');
      return;
    }

    // 显示正在保存的loading消息
    const hide = message.loading(`正在保存 ${itemsArray.length} 个观点条目...`, 0);
    console.log('[Viewpoint] 显示loading消息');
    
    try {
      const remark = '';
      console.log('[Viewpoint] 调用API - projectId:', projectId, 'remark:', remark);
      
      const result = await exportViewpointItemsToZip(projectId, remark);
      
      console.log('[Viewpoint] API调用成功:', result);
      ...
      console.log('[Viewpoint] 保存成功');
    } catch (error) {
      console.error('[Viewpoint] 保存失败:', error);
      console.error('[Viewpoint] 错误详情:', error.response?.data);
  ...
  const handleDownload = () => {
    ...
    link.download = `Project${projectId}_${selectedItem?.name || 'viewpoint'}_${dateStr}.md`;
  ...
  const loadVersions = async () => {
    ...
    try {
      console.log('[loadVersions] 开始加载观点版本列表, projectId:', projectId);
      // 查询versions表的所有记录，前端过滤viewpoint-batch
      ...
      // 过滤出item_type为viewpoint-batch的记录
      const filteredData = data.filter(v => v.item_type === 'viewpoint-batch');
      console.log('[loadVersions] 过滤后的观点版本列表:', filteredData);
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size={4}>
          ...
          />
          <Popconfirm
            title={t('viewpoint.confirmDelete')}
  ...
  return (
    <div className="requirement-item-panel">
            ...
            <div className="panel-header">
              <Space direction="vertical" style={{ width: '100%' }} size="small">
                ...
                <Button
                  ...
                  size="small"
                >
                  {t('viewpoint.versionList')}
                </Button>
              </Space>
            </div>
            <div className="panel-list-title">
              <span>{t('viewpoint.itemList')}</span>
              <Button 
                type="text" 
                size="small" 
        ...
        {!leftPanelCollapsed && (
          <div className="requirement-list">
            {loading ? (
              ...
              <Empty description="暂无观点" style={{ marginTop: '20px' }} />
            ) : (
              items.map(item => {
      ...
      {/* 新建观点对话框 */}
      <Modal
        title={t('viewpoint.createItemTitle')}
        open={isModalVisible}
      ...
      {/* 版本一览对话框 */}
      <Modal
        title={t('viewpoint.versionListTitle')}
      ...
      {/* 编辑备注对话框 */}
      <Modal
        title={t('viewpoint.editRemarkTitle')}
      ...
      {/* 编辑名称对话框 */}
      <Modal
        title={t('viewpoint.editItemTitle')}
...
export default ViewpointItemPanel;
